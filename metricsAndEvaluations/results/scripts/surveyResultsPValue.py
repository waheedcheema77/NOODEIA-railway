...
def calculate_statistics(baseline_data, proposed_data):
    ...
    for b_data, p_data in zip(baseline_data, proposed_data):
        ...
        t_stat, p_value = stats.ttest_rel(b_data, p_data)
...
def prepare_table_data(questions, results):
    ...
    for i, (question, result) in enumerate(zip(questions, results)):
        ...
        p_value_cell = f"{result['p_value']:.4f}"
...
def create_table(ax, table_data):
    headers = ["Question", "Baseline Avg. (SD)", "Proposed Avg. (SD)", "p-value", "Significance"]
    
    table = ax.table(cellText = table_data, colLabels = headers, 
                    cellLoc = "left", loc = "center",
                    colWidths = [0.35, 0.18, 0.18, 0.12, 0.12])
...
def add_significance_note(fig):
    fig.text(0.98, 0.02, "** p < 0.01, * p < 0.05", 
...
def generate_table(csv_path, output_path):
    ...
    add_significance_note(fig)
    
    save_figure(output_path)

if __name__ == "__main__":
    csv_path = "../surveyResults.csv"
    output_path = "../visualizations/surveyResultPValueVisualization.png"
